...
class IMClient:
    ...
    def connect(self):
        try:
            ...
            logger.warning(f"IMClient 连接失败: {e}")
            return False
    ...
    def register(self, bind_number, watchid, chipid):
        if not self.socket:
            logger.warning("IMClient 未连接到服务器")
            return False
...
def send_im_message(bind_number, watchid, chipid, imfriendid, imaccountid, content):
    ...
    try:
        ...
        # 任何分支（含异常）都保证 disconnect 被调用，避免 socket 泄漏
        try:
            client.disconnect()
        except Exception:
            pass
